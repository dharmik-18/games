document.addEventListener('DOMContentLoaded', function () {
   ...
   const dislayCurrentPlayer = document.querySelector('#current-player');
   let currentplayer = 1;
   ...
   for (let i = 0, len = squares.length; i < len; i++) {
      ...
      (function (index) {
         ...
         squares[i].onclick = function() {
            //if the square below your current then you can go top
            if (squares[index + 7].classList.contains('taken')) {
               if (currentplayer === 1) {
                  ...
                  //chnage the player
                  currentplayer = 2;
                  dislayCurrentPlayer.innerHTML = currentplayer;
               ...
               ///if the square below your current is not taken you cant go there

            }
            else alert("can't go there");
         }
      })(i);
//check the board for win or losss
      function checkBoard() {
         //make const that show al wining array
         const winningArrays = [
         ...
         // now take the4 values in each winningArray and plug them into the squares
         for (let y = 0; y <winningArrays.length; y++){
            const square1 = squares[winningArrays[y][0]];
            const square2 = squares[winningArrays[y][1]];
            const square3 = squares[winningArrays[y][2]];
            const square4 = squares[winningArrays[y][3]];
//now check all them to have class name player-one
